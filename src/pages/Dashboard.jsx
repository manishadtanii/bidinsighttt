...
function Dashboard() {
 ...
 const handleSort = (field) => {
  setFilters((prev) => {
    ...
    if (current === `-${field}`) {
      // First click -> Second click: Ascending to Descending
      newOrder = field;
    }
    // } else if (current === `-${field}`) {
    //   // Second click -> Third click: Descending to Default (no sorting)
    //   newOrder = "";
    // } 
    else {
      // Default/Other field -> First click: Set to Ascending
      newOrder = `-${field}`;
    }
    
    const updatedFilters = { ...prev, ordering: newOrder };
    
    // Also update appliedFilters immediately for API call
    setAppliedFilters(updatedFilters);
    
    console.log(`🔥 Sort clicked: ${field}, Current: ${current}, New: ${newOrder}`);
