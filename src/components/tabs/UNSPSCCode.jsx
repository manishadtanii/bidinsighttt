...
const UNSPSCCode = ({ filters, setFilters, onApply }) => {
  ...
  const filteredData = unspscData.filter((item) =>
    ...
    <div className="min-h-screen flex flex-col justify-between p-10 ps-14 overflow-y-auto">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-p font-medium font-inter">
          Selected UNSPSC Codes <span className="text-primary">({selected.length})</span>
        </h2>
        {selected.length > 0 && (
      ...
      {/* API Loading/Error */}
      {loading && <p className="text-center text-gray-500 py-10">Loading UNSPSC codes...</p>}
      {error && <p className="text-center text-red-500 py-10">{error}</p>}

      {/* List */}
      {!loading && !error && (
        <div className="border-[#273BE280] border-[2px] rounded-[10px] mt-6 overflow-y-auto">
          <div className="text-p font-medium font-inter border-b px-4 py-3">Available UNSPSC Codes</div>
